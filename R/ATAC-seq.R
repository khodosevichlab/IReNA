...
find_motifs <- function(motif, step = 20, fimodir, outputdir, Motifdir, sequencedir) {
  ...
  tmp <- matrix(seq(round_any(nrow(motif), step, f = ceiling)), nrow = 500) %>% 
    {lapply(seq_len(ncol(.)), function(i) .[,i])}
  
  seq(1:length(tmp)) %>% 
    lapply(\(i) {
      if (nrow(motif) - max(tmp[[i]]) > step) {
        motif.tmp <- motif %>% 
          slice(tmp[[i]]) %>% 
          pull(1)
        
        data.frame(fimodir,
                   " --parse-genomic-coord --max-stored-scores 2000000  --text >",
                   outputdir, 
                   motif.tmp,
                   ".txt ", 
                   ' ', 
                   ...
                   sequencedir,';') %>% 
          write.table(
            paste0(outputdir, "/Fimo", i, ".sh"), 
            row.names = FALSE, 
            col.names = FALSE, 
            quote = FALSE)
      } else {
        motif.tmp <- motif %>% 
          slice(min(i):nrow(motif)) %>% 
          pull(1)
  ...
  sapply(seq(1:length(tmp)), \(i) paste0("nohup sh ",outputdir,"/Fimo",i,".sh &")) %>% 
    paste(collapse = " ") %>% 
    write.table(paste0(outputdir, "/Fimo_All.sh"), 
                row.names = FALSE,
                col.names = FALSE, 
                quote = FALSE)
}
